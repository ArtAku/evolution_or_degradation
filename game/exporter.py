...
class RedisMapMonitor(MapMonitor):
    ...
    def export_map(self) -> None:
        ...
        for i, cells_raw in enumerate(cells):
            self.redis_client.set(f"{i}_t", str([c.temperature / temperature_range[1] for c in cells_raw]))
            self.redis_client.set(f"{i}_h", str([c.hardness / hardness_range[1] for c in cells_raw]))
            self.redis_client.set(f"{i}_e", str([c.current_energy / energy_range[1] for c in cells_raw]))
            self.redis_client.set(f"{i}_i", str([c.energy_income / mx for c in cells_raw]))
